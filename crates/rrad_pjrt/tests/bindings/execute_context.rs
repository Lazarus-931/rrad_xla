...
mod execute_context_bindings_tests {
    ...
    use rrad_pjrt::executable::{
        ...
        PJRTSendCallbackInvocation, PJRTSendCallbackRegistration,
    };
    use rrad_pjrt::execute_context::PJRTExecuteContext;
    use std::ptr::null_mut;
    use std::sync::atomic::{AtomicBool, Ordering};

    const SEND_CHANNEL_ID: i64 = 1;
    const RECV_CHANNEL_ID: i64 = 2;
    pub const _BUFFER_SHAPE_DIM: &[i64] = &[1, 2, 3, 4, 5];
...
func.func @main(%arg0: tensor<f32>) -> (tensor<f32>, tensor<f32>) {
  ...
  return %2, %0 : tensor<f32>, tensor<f32>
    ...
    fn execute_with_context_fails_on_missing_args() -> TestResult {
        ...
        if result.is_ok() {
            return Err(
                "expected execute_with_context to fail with missing arguments"
                    .to_string()
                    .into(),
            );
        }
    ...
    fn multi_output_execute_with_context_smoke() -> TestResult {
        ...
        if outputs.len() != 2 {
            return Err(
                format!("expected exactly two output buffers, got {}", outputs.len()).into(),
            );
        }
    ...
    fn execute_with_options_rejects_callback_count_mismatch() -> TestResult {
        ...
        if result.is_ok() {
            return Err(
                "expected execute_with_options to reject callback count/registration mismatch"
                    .to_string()
                    .into(),
            );
        }
    ...
    fn execute_context_into_raw_manual_destroy_smoke() -> TestResult {
        ...
        if !err.is_null() {
            return Err("PJRT_ExecuteContext_Destroy failed for raw execute context"
                .to_string()
                .into());
        }
        Ok(())
    ...
    fn execute_context_send_callback_success_smoke() -> TestResult {
        ...
        let client = rt.create_client()?;
        let executable =
            client.compile_on_topology_code(MODULE_SEND_CALLBACK, "mlir", &[], None)?;
    ...
    fn execute_context_recv_callback_smoke() -> TestResult {
        ...
        let executable =
            client.compile_on_topology_code(MODULE_RECV_CALLBACK, "mlir", &[], None)?;
    ...
    fn callback_lifetime_survival_smoke() -> TestResult {
        ...
        let client = rt.create_client()?;
        let executable =
            client.compile_on_topology_code(MODULE_SEND_AND_RECV, "mlir", &[], None)?;
        let buffer = client.buffer_from_host_slice_copy(
            &[41.0f32],
            PJRT_Buffer_Type_PJRT_Buffer_Type_F32,
            &[],
