...
use rrad_pjrt::rrad_pjrt::copy_to_device_stream::PJRTCopyToDeviceStreamRef;
use rrad_pjrt::rrad_pjrt::executable::{
    PJRTRecvCallbackInvocation, PJRTRecvCallbackRegistration, PJRTSendCallbackRegistration,
};
...
fn recv_chunk_callback(inv: PJRTRecvCallbackInvocation) -> Result<(), String> {
    RECV_CALLBACK_HIT.store(true, Ordering::SeqCst);

    let rt_ptr = RUNTIME_PTR.load(Ordering::SeqCst) as *const PjrtRuntime;
    if rt_ptr.is_null() {
        return Err("runtime pointer not initialized".to_string());
    }
    let rt = unsafe { &*rt_ptr };

    let stream = PJRTCopyToDeviceStreamRef::new(rt, inv.stream);

    let total_bytes = stream.total_bytes().map_err(|e| e.to_string())?;
    let current_bytes = stream.current_bytes().map_err(|e| e.to_string())?;
    let granule_size = stream.granule_size().map_err(|e| e.to_string())?;

    if total_bytes <= 0 {
        return Err(format!("expected total_bytes > 0, got {total_bytes}"));
    }
    ...
    if granule_size <= 0 {
        return Err(format!("expected granule_size > 0, got {granule_size}"));
    }
    ...
    stream
        ...
        .map_err(|e| format!("add_chunk failed: {e}"))
}
...
fn add_chunk_smoke() -> TestResult {
    ...
    if outputs.len() != 1 {
        return Err(format!("expected exactly one output buffer, got {}", outputs.len()).into());
    }
